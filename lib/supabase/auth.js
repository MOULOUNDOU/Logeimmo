...
export const getCurrentUser = async () => {
  try {
    ...
    // AbortError = annulation normale (navigation/unmount). On évite de polluer la console.
...
export const updateProfile = async (userId, updates) => {
  try {
    ...
    const { error: annonceUpdateError } = await supabase
      .from('annonces')
      .update({
        created_by_nom: updates.nom,
        created_by_photo: updates.photoProfil || null,
        updated_at: new Date().toISOString()
      })
      .eq('created_by', userId)

    if (annonceUpdateError) throw annonceUpdateError

    if (typeof window !== 'undefined') {
      try {
        const authDataRaw = localStorage.getItem(STORAGE_KEY_AUTH)
        if (authDataRaw) {
          ...
          if (authData?.user?.id === userId) {
            const nextAuthData = {
              ...
              user: {
                ...
                nom: updates.nom,
                telephone: updates.telephone,
                photoProfil: updates.photoProfil,
                coverPhoto: updates.coverPhoto
              }
            }
            localStorage.setItem(STORAGE_KEY_AUTH, JSON.stringify(nextAuthData))
          }
        }
      } catch (e) {
